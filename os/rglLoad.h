...
#define PROCS                                                                  \
  PROC(void, glShaderSource, GLuint shader, GLsizei count,                     \
  ...
  PROC(void, glBindAttribLocation, GLuint program, GLuint index,               \
       const GLchar *name)                                                     \
  PROC(void, glLinkProgram, GLuint program)                                    \
  PROC(void, glBindBuffer, GLenum target, GLuint buffer)                       \
  PROC(void, glBufferData, GLenum target, GLsizeiptr size, const void *data,   \
       GLenum usage)                                                           \
  PROC(void, glEnableVertexAttribArray, GLuint index)                          \
  PROC(void, glVertexAttribPointer, GLuint index, GLint size, GLenum type,     \
  ...
  PROC(void, glBufferSubData, GLenum target, GLintptr offset, GLsizeiptr size, \
       const void *data)                                                       \
  PROC(void, glGetShaderiv, GLuint shader, GLenum pname, GLint *params)        \
  PROC(void, glGetShaderInfoLog, GLuint shader, GLsizei bufSize,               \
       GLsizei *length, GLchar *infoLog)                                       \
  PROC(void, glGetProgramiv, GLuint program, GLenum pname, GLint *params)      \
  PROC(void, glGetProgramInfoLog, GLuint program, GLsizei bufSize,             \
       GLsizei *length, GLchar *infoLog)                                       \
  ...
  PROC(void, glUniformMatrix4fv, GLint location, GLsizei count,                \
       GLboolean transpose, const GLfloat *value)                              \
  PROC(void, glTexImage2D, GLenum target, GLint level, GLint internalformat,   \
       GLsizei width, GLsizei height, GLint border, GLenum format,             \
       GLenum type, const void *pixels)                                        \
  PROC(void, glActiveTexture, GLenum texture)                                  \
  PROC(void, glDebugMessageCallback, void *callback, const void *)             \
  PROC(void, glDrawElements, GLenum mode, GLsizei count, GLenum type,          \
       const void *indices)                                                    \
...
// Each lazy loaded OpenGL function call might be slighty slower due to an
// additional condition check, but this is not a performance critical
// application. Change this to something better when performance issues occur.
#define LAZY_LOAD_PROC(name)                                                   \
  (name##SRC ? name##SRC                                                       \
             : (name##PROC)load_proc_or_die(#name, (gl_proc *)&name##SRC))
